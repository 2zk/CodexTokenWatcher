...
Claude Fable / seven_day / 7日（週次）: 残量 95%（使用 5%）/ リセット 2026-09-29 10:30:00
```

キャッシュが 300 秒以上古い、またはいずれかの期間のリセット時刻を過ぎている場合は出力末尾に「※ 最新データが取得できていません」と注記する。stale 値では通知しない。JSON 出力には `stale` フラグ（boolean）が含まれる。
